...
export const getInitialInfoBinance = async () => {
    const response = await fetch(BINANCE_ENDPOINT.API_EXCHANGEINFO, {method: FETCH_METHOD.GET, headers: header, body: null})
...
export const binance_spot_startTickerWebsocket = async (codes: string[], options: any, listener: any) => {
    ...
    const response = await fetch(BINANCE_ENDPOINT.API_TICKER, {method: FETCH_METHOD.GET, headers: header, body: null})
    if (!response) return null;
    if (response.status !== 200) {
        console.log("[BINANCE] getInitialInfo error: ", response);
        return null;
        ...
        try {
            ...
            for (const ticker of tickerRes) {
                ...
                let aggTradeInfo: IAggTradeInfo = {
                    ...
                    bestBidPrice: parseFloat(ticker.b),
                    bestBidQty: parseFloat(ticker.B),
                    bestAskPrice: parseFloat(ticker.a),
